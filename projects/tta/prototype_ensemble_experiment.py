...
def mutual_info_loss(probs):
    ...
    probs = probs.permute(1, 0, 2) # (bz, n_models, d)
...
@dataclass
class MIEnsembleConfig(BaselineConfig):
    """Configuration for the experiment."""
    name: str = "MI_ensemble_gn_5mdls_test"
    resume: bool = True
    debug: bool = False
    use_wandb: bool = True
...
class MIEnsemblExperiment(BaselineExperiment): 
    config_class = MIEnsembleConfig
    config: MIEnsembleConfig
    ...
    def setup_model(self):
        models = [super(MIEnsemblExperiment, self).setup_model().cuda()
        ...
        # model = super(MIEnsemblExperiment, self).setup_model().cuda()
    ...
    def run_epoch(self, loader, train=True, desc="train"):
        with torch.no_grad() if not train else torch.enable_grad():
            ...
            for i, batch in enumerate(tqdm(loader, desc=desc)):
                ...
                diversity_loss = self.config.var_coeff*var_loss + self.config.cov_coeff*cov_loss                
                MI_loss = mutual_info_loss(F.softmax(logits, dim=-1))
                loss = sum(ce_losses) + diversity_loss + self.config.mi_coeff*MI_loss
